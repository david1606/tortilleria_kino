...
public class TempoActivity extends AppCompatActivity {
    ...
    private void writeClients() {
        ...
        for (int i = 0; i < clients.size(); i++) {
            db.collection("clients")
                    .add(clients.get(i)).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
                @Override
                public void onSuccess(DocumentReference documentReference) {
                    Toast.makeText(TempoActivity.this, "added", Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
    ...
    private void writeArticles() {
        ...
        for (int i = 0; i < articles.size(); i++) {
            ...
            db.document("articles/article_" + numb).set(articles.get(i))
                    .addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void aVoid) {
                            Toast.makeText(TempoActivity.this, "Saved", Toast.LENGTH_SHORT).show();
                        }
                    }).addOnFailureListener(new OnFailureListener() {
                @Override
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(TempoActivity.this, "Error: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
